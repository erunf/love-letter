...
interface OnlineRoundEndScreenProps {
  snapshot: GameSnapshot;
  yourPlayerId: string;
  send: (msg: ClientMessage) => void;
}

export function OnlineRoundEndScreen({ snapshot, yourPlayerId, send }: OnlineRoundEndScreenProps) {
  const result = snapshot.lastRoundResult;
  const isHost = snapshot.hostId === yourPlayerId;

  if (!result) return null;

  const winner = snapshot.players.find((p) => p.id === result.winnerId);
  const spyBonusPlayer = result.spyBonusPlayerId
    ? snapshot.players.find((p) => p.id === result.spyBonusPlayerId)
    : null;

  const reasonText = {
    lastStanding: 'Last one standing!',
    highestCard: 'Highest card value!',
    tiebreak: 'Won by tiebreak!',
  }[result.reason];
    ...
    <div className="bg-royal min-h-screen flex items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          >
            {result.winnerName}
            {result.winnerId === yourPlayerId && ' (You!)'}
          </p>
        ...
        {spyBonusPlayer && (
          ...
          >
            <p className="text-sm font-semibold" style={{ color: THEME.gold }}>
              {spyBonusPlayer.name} earns a Spy bonus token!
            </p>
          </motion.div>
        )}
        ...
        >
          <p
            ...
            style={{ color: THEME.gold, fontFamily: "'Cinzel', serif", fontSize: 10 }}
          >
            Favor Tokens ({snapshot.tokensToWin} to win)
          </p>
